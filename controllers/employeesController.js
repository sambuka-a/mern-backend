...
const updateEmployee = async (req, res) => {
    ...
    if (!employee) {
        return res.status(204).json({ "message": `Ne employee mathes ID ${req.body.id}` });
    }
...
const deleteEmployee = async (req, res) => {
    ...
    const employee = await Employee.findOne({_id: req.body.id}).exec()
    if (!employee) {
        return res.status(204).json({ "message": `No employee mathes ID ${req.body.id}`});
    }
...
const getEmployee = async (req, res) => {
    ...
    if (!employee) {
        return res.status(204).json({"message": `No employee mathes ID ${req.params.id}`});
    }
    res.json(employee);
}
